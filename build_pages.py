...
# ---------------- CHIP GALLERY ----------------
def dtile(img, label_key, label_en, idx_label, process=None, show_label=True):
    path = f"assets/chips/{img}"
    meta = f"{idx_label} &middot; {process}" if process else idx_label
    label_html = f'<h4 data-i18n="{label_key}">{label_en}</h4>\n            ' if show_label else ""
    return f"""        <article class="tile">
          ...
          <div class="tile-caption">
            {label_html}<div class="tile-meta">{meta}</div>
          </div>
        </article>"""
